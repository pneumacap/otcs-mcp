...
**Tips:** Use include_highlights for context. Use location_id to scope to a subtree. Use filter_type to restrict by object type.`,
    schema: {
      ...
      properties: {
        ...
        location_id: {
          type: 'number',
          description: 'Scope search to a specific folder/workspace subtree by its node ID',
        },
  ...
  // ==================== Folders (1 tool) ====================
  {
    ...
    schema: {
      ...
      properties: {
        ...
        path: {
          type: 'string',
          description: 'Optional: Create nested path (e.g., "2024/Q1/Reports")',
        },
  ...
  // ==================== Workspace Roles (1 tool) ====================
  {
    ...
    schema: {
      ...
      properties: {
        action: {
          type: 'string',
          enum: ['get_roles', 'get_members', 'get_role_members', 'add_member', 'remove_member'],
          description: 'Action',
        },
        workspace_id: { type: 'number', description: 'Workspace ID' },
    ...
    schema: {
      ...
      properties: {
        mode: {
          ...
          description: 'Query mode: by_status for dashboard view, active for running instances',
        },
        status: {
          type: 'string',
        ...
        wfretention: {
          type: 'number',
          description: 'Filter on workflow completion date in days (for by_status mode)',
        },
      },
    },
  },
    ...
    schema: {
      ...
      properties: {
        mode: {
          ...
          description: 'Start mode. Use "direct" (default) for most cases.',
        },
  ...
  },
  {
    ...
    description:
      'Manage draft workflow forms (advanced). Actions: get_form, update_form, initiate.',
    schema: {
      type: 'object',
      properties: {
        action: {
  ...
  // ==================== Permissions (1 tool) ====================
  {
    ...
    schema: {
      ...
      properties: {
        action: {
          type: 'string',
          enum: ['get', 'add', 'update', 'remove', 'effective', 'set_owner', 'set_public'],
          description: 'Action',
        },
        node_id: { type: 'number', description: 'Node ID' },
        ...
        apply_to: {
          type: 'number',
          enum: [0, 1, 2, 3],
          description: '0=This Item, 1=Sub-Items, 2=This & Sub, 3=Immediate Children',
        },
      },
      required: ['action', 'node_id'],
    },
          ...
          items: {
            ...
            properties: {
              ...
              perm: {
                type: 'number',
                enum: [1, 2, 3, 4],
                description: '1=Viewer, 2=Collaborator, 3=Manager, 4=Owner',
              },
  ...
  // ==================== Tree Browsing & Creation (2 tools) ====================
  {
    ...
    description:
      'Recursively browse a folder hierarchy and return the full tree structure in a single call.',
    schema: {
      type: 'object',
      properties: {
        ...
        folders_only: {
          type: 'boolean',
          description: 'If true, only include folders in the tree (default true)',
        },
      },
      required: ['folder_id'],
    },
  },
  {
    ...
    description:
      'Create an entire folder tree structure in a single call. Existing folders are reused rather than duplicated.',
    schema: {
      type: 'object',
      properties: {
        parent_id: {
          type: 'number',
          description: 'The parent folder ID under which to create the structure',
        },
        folders: {
          type: 'array',
