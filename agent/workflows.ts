    ...
    switch (action.type) {
      ...
      case "smart_search": {
        ...
        if (Array.isArray(rawQueries)) {
          ...
          // LLM sometimes returns a JSON-stringified array instead of a real array
        ...
        if (Array.isArray(rawFilters)) {
          filterKeywords = rawFilters.map((k) => String(k).toLowerCase());
        } else if (typeof rawFilters === "string") {
          const trimmedFilters = rawFilters.trim();
          if (trimmedFilters.startsWith("[")) {
            try {
              const parsed = JSON.parse(trimmedFilters);
              filterKeywords = Array.isArray(parsed) ? parsed.map((k: unknown) => String(k).toLowerCase()) : [];
            } catch {
              filterKeywords = trimmedFilters.split(",").map((s) => s.trim().toLowerCase()).filter(Boolean);
            }
          } else {
            filterKeywords = trimmedFilters.split(",").map((s) => s.trim().toLowerCase()).filter(Boolean);
          }
